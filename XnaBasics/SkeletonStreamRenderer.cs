    ...
    public class SkeletonStreamRenderer : Object2D
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (skeletonDrawn)
            {
                ...
                if (playerId == 0)
                {
                    bool squeletteFound = false;
                    int i = 0;
                    while (!squeletteFound && i < 6)
                    {
                        if (skeletonData[i].TrackingId != 0)
                        {
                            squeletteFound = true;
                            playerId = skeletonData[i].TrackingId;
                        }
                        i++;
                    }
                }
                else
                {
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        if (skeletonData[i].TrackingId == playerId)
                        {
                            _x = skeletonData[i].Joints[JointType.HipCenter].Position.X;
                            _y = skeletonData[i].Joints[JointType.Head].Position.Y;                           
                            skelCoords1 = "skel head X = " + _x.ToString() + ", Y = " + _y.ToString();
                            player1Coords.X = _x;
                            player1Coords.Y = _y;
                            playerFound = true;
                        }
